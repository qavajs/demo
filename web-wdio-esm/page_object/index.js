import { locator } from '@qavajs/steps-wdio/po.js';
...
export class App {
    ...
    Product = locator.template(text => `//*[@class="inventory_item"][contains(.,"${text}")]`).as(Product);
    CartButton = locator('a.shopping_cart_link');
    CartBadge = locator('.shopping_cart_badge');
    CartItems = locator('.cart_item');
    CartItem = locator.template(text => `//*[@class="cart_item"][contains(.,"${text}")]`).as(CartItem);
